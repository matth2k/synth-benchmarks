...
if __name__ == "__main__":
    ...
    avg_build_times = [
        sum(times) / len(times)
        for times in [iteration_dict[iter] for iter in iterations]
    ]
    derivative_build_times = [0] + [
    ...
    plt.plot(
        ...
        derivative_build_times[:max_iteration],
        linewidth=2.5,
    )
    ...
    plt.savefig(
        os.path.join(graphs_dir, "runtime_derivative.pdf"),
        format="pdf",
        bbox_inches="tight",
    )
    ...
    plt.yticks(fontsize=19)
    plt.savefig(
        os.path.join(graphs_dir, "runtime.pdf"), format="pdf", bbox_inches="tight"
    )
