...
for value in $values; do
    ...
    make -f run.mk alus_eqmap_results.json TOOL=eqmap INFO=yosys33-t-${value} -j 10 TIMEOUT=${value}
    mv alus_eqmap_results.json yosys33_no_decomp_t_${value}.json
    rm -rf *.rpt
    ...
    source /work/shared/common/yosys/33/setup.sh 
    make -f run.mk alus_eqmap_results.json TOOL=eqmap INFO=yosys33-decomp-t-${value} -j 10 TIMEOUT=${value} EXTRA_FLAGS="--decomp"
    mv alus_eqmap_results.json yosys33_decomp_t_${value}.json
    rm -rf *.rpt
done
